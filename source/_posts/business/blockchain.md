...
* 资源: 无状态,我只要知道文件的哈希值,以及文件的碎片存储在哪个peer就可以了.文件没有版本演化.
* 账本: 有状态,一笔资金从产生到不断的转账的所有的历史都要存储下载,并且历史都要无误的,不能被篡改的.那就要解决一个问题,`准确的历史的共识`谁来产生,谁说的话算数.矿工,POW...随即来解决这个问题.带来的问题是损耗大量的电力,资源,因为他的本质是`在全球的计算节点中找出来一个老大,老大的话就是大家的共识`...等等等就不继续展开了
...
但是没有规范的数据呢,例如二进制文件.那必须得有一个算法把文件之间的差异整成一个diff文件,这个二进制比上个二进制增加/减少了哪些东西,这个问题解决了,那么再配合DPOS超级节点便可以做到`分布式存储有状态的非版本账本数据`
