...
* 币币交易: usdt或btc或eth 买卖 其他币种

币币交易: 即使涉及不同的资产,交易也都是限制在单一的区块链里,一个另类币的交易和另外的其他另类币的交易历史`没有任何关系可以相互参考`

有个聪明的办法可以做到,这用到了密码学的承诺和锁定时间的存储

截至2015年,还没有人用到.相反所有的加密货币都是在传统的中心化的交易系统力交易  
* 该协议复杂,不便和缓慢
* 防止偷盗货币,但是不能防范服务性攻击
...
但是上面的物理产权是:  汽车制造商永久地把公钥存储在了汽车里

如果运用到了区块链,就会变成:

当汽车在组装厂组装的时候,遥控器里第一个用户的`公开秘钥`通过特殊操作加入`区块链`.这辆智能汽车也把该`特殊操作的ID`写入它`自带的程序`

当汽车更改所有权的时候 -> 从装配车厂 -> 质量监控室 -> 运输人员 -> 汽车经销商 -> 第一个拥有者 同时更新到区块链

当汽车通过区块链转移的时候,前车主的遥控器无法工作新车主的遥控器获得开门和启动引擎的权利

但是支付上面也有问题:,如果A想买B的汽车,  1.B收到了钱不转移汽车的所有权 2. B转移了所有权A不给钱, 那这时候需要区块链技术来解决:

支付的货币和汽车的拥有权同时存在相同的区块链,A和B就会产生一个不可分割的交易.  A的财产和B的所有权,双方进行签名,一旦签名就对整个区块链进行广播,A只要等到几次预设的确认就能获得这辆车
