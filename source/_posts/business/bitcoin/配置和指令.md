...
查询某个高度区块信息
```bash
alias bitcoind="bitcoind -rpcuser=ubuntu -rpcpassword=ubuntu"

alias bitcoin-cli="bitcoin-cli -rpcuser=ubuntu -rpcpassword=ubuntu"

# 出一个块
bg1() {bitcoin-cli generate 1}

# 获取区块链所有的信息
ball() {bitcoin-cli getblockchaininfo}

# 获取指定指定高度的信息
binfo() {HASH=`bitcoin-cli getblockhash $1` && bitcoin-cli getblock $HASH}

# 获取指定高度的coinbase交易 Hash
bbase() {binfo $1 | python -c 'import json,sys;obj=json.load(sys.stdin);print (obj["tx"][0])'}

# 获取指定高度的coinbase交易的raw数据
brawt() {bitcoin-cli getrawtransaction `bbase $1`}

# 解码指定高度的coinbase交易
bt() {bitcoin-cli decoderawtransaction `brawt $1`}

# 不通过raw来解析交易
btinfo() {bitcoin-cli gettransaction `bbase $1`}
