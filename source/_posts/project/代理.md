...
client
```bash
cd /media/yq/ST1000DM003/linux/env/kcptun

export SS_PASSWD=xxx

./client_linux_amd64 -l :1080 -r dgc1:4002 --mode fast2 -crypt aes --key $SS_PASSWD > /dev/null 2>&1
```
...
```bash
cd ~/reference
git clone https://github.com/yqsy/recipes
cd recipes/httpproxy

nohup go run httpproxy.go localhost:445 > /dev/null 2>&1 &

export SS_PASSWD=xxx

cd ~/env/kcp
nohup ./server_linux_amd64 -t "localhost:445" -l ":4002" -mode fast2 --crypt aes --key $SS_PASSWD > /var/log/kcptun/kcptun3.log 2>&1 &
