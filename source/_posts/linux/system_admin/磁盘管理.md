...
- [5. 文件系统与目录树的关系(挂载)](#5-文件系统与目录树的关系挂载)
    - [5.1. 默认CentOS环境中,读写频繁的目录](#51-默认centos环境中读写频繁的目录)
- [6. various file systems](#6-various-file-systems)
    - [6.1. 区别](#61-区别)
    - [6.2. ReiserFS文件系统](#62-reiserfs文件系统)
    - [6.3. xfs(centos7默认)](#63-xfscentos7默认)
    - [6.4. ext3的3种日志模式](#64-ext3的3种日志模式)
- [7. Virtual Filesystem Switch](#7-virtual-filesystem-switch)
- [8. 磁盘划分标准](#8-磁盘划分标准)
- [9. Logical Volume Manager (逻辑卷管理)](#9-logical-volume-manager-逻辑卷管理)
    - [9.1. 概念](#91-概念)
    - [9.2. 命令整理](#92-命令整理)
    - [9.3. 磁盘快照](#93-磁盘快照)
    - [9.4. LVM thin volume 动态调整磁盘使用率](#94-lvm-thin-volume-动态调整磁盘使用率)
- [10. 软链接/硬链接](#10-软链接硬链接)
- [11. Linux文件系统的asyncchronously](#11-linux文件系统的asyncchronously)
- [12. 案例](#12-案例)
    - [12.1. 树莓派fdisk分割u盘](#121-树莓派fdisk分割u盘)
        - [12.1.1. 出错经历(分割了sda1)](#1211-出错经历分割了sda1)
    - [12.2. 树莓派扩大/dev/root容量](#122-树莓派扩大devroot容量)
    - [12.3. 树莓派磁盘LVM虚拟卷(没真实的磁盘啦)](#123-树莓派磁盘lvm虚拟卷没真实的磁盘啦)
        - [12.3.1. 添加新的物理卷到卷组](#1231-添加新的物理卷到卷组)
    - [12.4. 树莓派两个u盘一个逻辑卷](#124-树莓派两个u盘一个逻辑卷)
    - [12.5. 树莓派gdisk分xfs vfat swap (GPT)](#125-树莓派gdisk分xfs-vfat-swap-gpt)
    - [12.6. 树莓派ext3误删文件恢复](#126-树莓派ext3误删文件恢复)
    - [12.7. 树莓派搭建NFS文件共享服务](#127-树莓派搭建nfs文件共享服务)
    - [12.8. 树莓派自动挂载两个U盘(UUID)](#128-树莓派自动挂载两个u盘uuid)
- [13. 其他命令](#13-其他命令)
- [14. 常用工具](#14-常用工具)
...
<a id="markdown-5-文件系统与目录树的关系挂载" name="5-文件系统与目录树的关系挂载"></a>
# 5. 文件系统与目录树的关系(挂载)

<a id="markdown-51-默认centos环境中读写频繁的目录" name="51-默认centos环境中读写频繁的目录"></a>
...
<a id="markdown-6-various-file-systems" name="6-various-file-systems"></a>
...
<a id="markdown-61-区别" name="61-区别"></a>
## 6.1. 区别
...
<a id="markdown-62-reiserfs文件系统" name="62-reiserfs文件系统"></a>
## 6.2. ReiserFS文件系统
* 高效,合理利用磁盘空间
* 先进的日志管理机制
...
<a id="markdown-63-xfscentos7默认" name="63-xfscentos7默认"></a>
## 6.3. xfs(centos7默认)
> 之前格式化过一个 70 TB 以上的磁盘阵列成为 ext4 文件系统，按下格式化，去喝了咖啡、吃了便当才回来看做完了没有... 所以，后来立刻改成 xfs 文件系统了。

<a id="markdown-64-ext3的3种日志模式" name="64-ext3的3种日志模式"></a>
## 6.4. ext3的3种日志模式
...
<a id="markdown-7-virtual-filesystem-switch" name="7-virtual-filesystem-switch"></a>
# 7. Virtual Filesystem Switch
![](http://ouxarji35.bkt.clouddn.com/snipaste_20170827_234052.png)

<a id="markdown-8-磁盘划分标准" name="8-磁盘划分标准"></a>
# 8. 磁盘划分标准
...
<a id="markdown-9-logical-volume-manager-逻辑卷管理" name="9-logical-volume-manager-逻辑卷管理"></a>
...
<a id="markdown-91-概念" name="91-概念"></a>
## 9.1. 概念
...
<a id="markdown-92-命令整理" name="92-命令整理"></a>
## 9.2. 命令整理

![](http://ouxarji35.bkt.clouddn.com/snipaste_20171026_131340.png)


<a id="markdown-93-磁盘快照" name="93-磁盘快照"></a>
## 9.3. 磁盘快照
```bash
# 备份
xfsdump
...
<a id="markdown-94-lvm-thin-volume-动态调整磁盘使用率" name="94-lvm-thin-volume-动态调整磁盘使用率"></a>
## 9.4. LVM thin volume 动态调整磁盘使用率



<a id="markdown-10-软链接硬链接" name="10-软链接硬链接"></a>
# 10. 软链接/硬链接
...
<a id="markdown-11-linux文件系统的asyncchronously" name="11-linux文件系统的asyncchronously"></a>
...
<a id="markdown-12-案例" name="12-案例"></a>
# 12. 案例

<a id="markdown-121-树莓派fdisk分割u盘" name="121-树莓派fdisk分割u盘"></a>
## 12.1. 树莓派fdisk分割u盘
...
<a id="markdown-1211-出错经历分割了sda1" name="1211-出错经历分割了sda1"></a>
### 12.1.1. 出错经历(分割了sda1)
...
<a id="markdown-122-树莓派扩大devroot容量" name="122-树莓派扩大devroot容量"></a>
...
<a id="markdown-123-树莓派磁盘lvm虚拟卷没真实的磁盘啦" name="123-树莓派磁盘lvm虚拟卷没真实的磁盘啦"></a>
...
<a id="markdown-1231-添加新的物理卷到卷组" name="1231-添加新的物理卷到卷组"></a>
### 12.3.1. 添加新的物理卷到卷组
...
<a id="markdown-124-树莓派两个u盘一个逻辑卷" name="124-树莓派两个u盘一个逻辑卷"></a>
## 12.4. 树莓派两个u盘一个逻辑卷
先清除逻辑卷,卷组,物理卷  
...
<a id="markdown-125-树莓派gdisk分xfs-vfat-swap-gpt" name="125-树莓派gdisk分xfs-vfat-swap-gpt"></a>
...
<a id="markdown-126-树莓派ext3误删文件恢复" name="126-树莓派ext3误删文件恢复"></a>
## 12.6. 树莓派ext3误删文件恢复
```bash
# google的开源项目
...
<a id="markdown-127-树莓派搭建nfs文件共享服务" name="127-树莓派搭建nfs文件共享服务"></a>
## 12.7. 树莓派搭建NFS文件共享服务


<a id="markdown-128-树莓派自动挂载两个u盘uuid" name="128-树莓派自动挂载两个u盘uuid"></a>
...
<a id="markdown-13-其他命令" name="13-其他命令"></a>
# 13. 其他命令
...
<a id="markdown-14-常用工具" name="14-常用工具"></a>
# 14. 常用工具
