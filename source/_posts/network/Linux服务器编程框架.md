...
- [3. 对于非阻塞I/O的理解](#3-对于非阻塞io的理解)
- [4. 两种高效的事件处理模式](#4-两种高效的事件处理模式)
    - [4.1. Reactor模式](#41-reactor模式)
    - [4.2. Proactor模式](#42-proactor模式)
    - [4.3. 模拟Proactor模式](#43-模拟proactor模式)
- [5. 两种高效的并发模式](#5-两种高效的并发模式)
    - [5.1. 半同步/半异步模式](#51-半同步半异步模式)
        - [5.1.1. 半同步/半反应堆](#511-半同步半反应堆)
        - [5.1.2. 高效的半同步/半异步模式](#512-高效的半同步半异步模式)
    - [5.2. 领导者/追随者模式](#52-领导者追随者模式)
...
* one thread/process per connection + 阻塞IO
* io-multiplexing + no-blocking (libev libevent netty nginx redis)
* 协程 (gevent go)
* actor 轻量级线程  (erlang)

<a id="markdown-3-对于非阻塞io的理解" name="3-对于非阻塞io的理解"></a>
# 3. 对于非阻塞I/O的理解

* 能用thread per connection 尽量用 (贴近人的思维)
* I/O复用 基于事件 (贴近机器的思维) 写起来/维护起来比较痛苦
* 一般都是使用网络库的
* 回调不能做阻塞操作
* go做的非常的好,贴近人的思维去写高性能的代码


我认为的历史进程:
* 1. 非阻塞I/O解决没有线程时处理多个文件描述符的问题
* 2. 线程(或进程) + 阻塞I/O (贴近人的思想,并发量不高) 
* 3. 非阻塞I/O event drive/event loop/reactor (贴近机器的思想,并发量高)
* 4. 协程 (同步方式写异步代码)


<a id="markdown-4-两种高效的事件处理模式" name="4-两种高效的事件处理模式"></a>
# 4. 两种高效的事件处理模式

服务器程序通常需要处理三类事件:`I/O事件`,`信号`,`定时事件`

<a id="markdown-41-reactor模式" name="41-reactor模式"></a>
## 4.1. Reactor模式
...
<a id="markdown-42-proactor模式" name="42-proactor模式"></a>
## 4.2. Proactor模式
...
<a id="markdown-43-模拟proactor模式" name="43-模拟proactor模式"></a>
## 4.3. 模拟Proactor模式
...
<a id="markdown-5-两种高效的并发模式" name="5-两种高效的并发模式"></a>
# 5. 两种高效的并发模式

<a id="markdown-51-半同步半异步模式" name="51-半同步半异步模式"></a>
## 5.1. 半同步/半异步模式
...
<a id="markdown-511-半同步半反应堆" name="511-半同步半反应堆"></a>
### 5.1.1. 半同步/半反应堆
...
<a id="markdown-512-高效的半同步半异步模式" name="512-高效的半同步半异步模式"></a>
### 5.1.2. 高效的半同步/半异步模式
...
<a id="markdown-52-领导者追随者模式" name="52-领导者追随者模式"></a>
## 5.2. 领导者/追随者模式
