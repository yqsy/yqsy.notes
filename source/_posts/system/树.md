...
<a id="markdown-1-辅助数据结构" name="1-辅助数据结构"></a>
# 1. 辅助数据结构
...
<a id="markdown-2-序列化和反序列化" name="2-序列化和反序列化"></a>
# 2. 序列化和反序列化
```python
def serialize(root):
    if not root:
    ...
    while queue:
        node = queue.popleft()
        if node:
            eles.append(str(node.val))
            queue.append(node.left)
            queue.append(node.right)
        else:
            eles.append('#')
 
    while eles and eles[-1] == '#':
        del eles[-1]
...
def deserialize(data):
    ...
    for l, r in zip(eles[0::2], eles[1::2]):
        node = queue.popleft()
        node.left = TreeNode(l) if l != '#' else None
        node.right = TreeNode(r) if r != '#' else None
...
<a id="markdown-4-层次遍历" name="4-层次遍历"></a>
# 4. 层次遍历
```python
def rescursive(root):
    if not root:
